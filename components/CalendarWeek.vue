<template>
  <div class="calendar-week-wrapper overflow-x-auto">
    <div
      class="calendar-week bg-white rounded-lg shadow-lg"
      :style="{ minWidth: `${200 + numDays * 120}px` }"
    >
      <!-- Week Header -->
      <div
        class="calendar-header grid border-b border-gray-200"
        :style="{
          gridTemplateColumns: `200px repeat(${numDays}, minmax(120px, 1fr))`,
        }"
      >
        <!-- User column header -->
        <div
      ...
      <div class="calendar-body">
        ...
        >
          ...
          <!-- Week Timeline Column (N days) - CSS Grid layout -->
          <div class="relative" :style="{ gridColumn: `2 / ${numDays + 2}` }">
...
const days = computed(() => getDateRange(props.startDate, props.endDate));
const numDays = computed(() => days.value.length);
...
// Compact header format for larger date ranges
const formatDayHeader = (day: Date): string => {
  if (numDays.value <= 7) {
    // 1 week: show day of month only
    return formatDayOfMonth(day);
  } else {
    // 2 weeks or more: show month/day
    return formatDate(day, 'M/d');
  }
};
    ...
    .map((userTicket) => {
      ...
      const isLaneFree = (
        ...
        for (let d = startCol; d < endCol; d++) {
          if (d >= 0 && d < numDays.value && occupancy[lane][d]) return false;
        }
        return true;
      };
      ...
      const occupyLane = (
        ...
        if (!occupancy[lane])
          occupancy[lane] = new Array(numDays.value).fill(false);
        for (let d = startCol; d < endCol; d++) {
          if (d >= 0 && d < numDays.value) occupancy[lane][d] = true;
        }
      };
      ...
      for (const ticket of sortedTickets) {
        ...
        const ticketEndDay = ticket.endDate
          ...
          : Math.min(todayDayIndex, numDays.value - 1); // extend to today, not end of range

        // Clamp to visible range
        let minCol = Math.max(0, ticketStartDay);
        let maxCol = Math.min(numDays.value, ticketEndDay + 1); // +1 because grid end is exclusive

        if (minCol >= numDays.value || maxCol <= 0) continue;
        ...
        const daysOpen = calculateActiveDays(ticket.stateSegments);
